...
{
    // Firebird doesn't support changing database on the fly, so we just return TRUE
    return TRUE;
}
	...
	{
		if ($this->qb_limit === FALSE || $this->qb_limit === NULL) {
			return $sql; // Don't apply limit if not set
		}
		...
		return preg_replace('/^(\s*SELECT\s)/i', '\\1' . $select . ' ', $sql);
	}
	// --------------------------------------------------------------------
	 ...
	 *
	 * Firebird handles charset via connection string, so we don't need to do anything here.
	 *
	...
	{
		// Firebird handles charset in the connection itself
		return TRUE;
	}
	 ...
	 * Escape Identifiers
	 *
	 * Escapes a string value with double quotes for Firebird/Interbase
	 *
	 * @param string $item
	 * @return string
	 */
	...
	{
		// If the item is already quoted, return as is
		if (strpos($item, '"') !== FALSE)
		{
			return $item;
		}

		// Wrap item in double quotes
	 ...
	 * FROM tables
	 *
	 * Escapes and compiles the FROM portion of the query
	 *
	 * @param array $tables
	 * @return string
	 */
